...
def _worker_loop(task_queue, result_queue, fasta_path, exon_parquet_path, trees_pkl_path):
    log("[WORKER] démarrage worker_loop")
    _init_worker(fasta_path, exon_parquet_path, trees_pkl_path)
    log("[WORKER] fin init_worker")
    while True:
        item = task_queue.get()
        if item is None:
            break
        ...
        try:
            ...
            start_put = time.time()
            log(f"[DEBUG] {transcript_id} - sending {len(recs)} recs one by one")

            for r in recs:
                result_queue.put(("record", r.id, str(r.seq), r.description))

            result_queue.put(("done", transcript_id, len(recs)))
            log(f"[DEBUG] {transcript_id} - put done in {time.time() - start_put:.2f} sec")

        except Exception as e:
            log(f"[ERROR WORKER] {transcript_id} - {e}")
            result_queue.put(("done", transcript_id, 0))
...
def _run_with_queue(tx_muts, num_workers, fasta_path, exon_parquet_path, trees_pkl_path):
    ...
    for i in range(num_workers):
        p = Process(
            target=_worker_loop,
            args=(task_queue, result_queue, fasta_path, exon_parquet_path, trees_pkl_path)
        )
        p.start()
        log(f"[MAIN] worker {i} démarré pid={p.pid}")
        workers.append(p)
    ...
    while received_done < total:
        item = result_queue.get()   # pas de timeout avec SimpleQueue
        tag = item[0]

        if tag == "record":
            _, rid, seq, desc = item
            yield SeqRecord(Seq(seq), id=rid, description=desc)

        elif tag == "done":
            _, transcript_id, nrecs = item
            received_done += 1
            log(f"[MAIN] reçu fin de {transcript_id} ({nrecs} recs)")

    log("[MAIN] Avant join workers")
    for p in workers:
        p.join()
        log(f"[MAIN] worker pid={p.pid} terminé exitcode={p.exitcode}")
    log("[MAIN] Après join workers")
