...
rule create_targets:
    ...
    log:
        "logs/{id}/targets.log"
    shell:
        "python scripts/V_fasta_generate_regions.py {input.fai} 1000000 > {output.targets}"
...
rule snpeff_annotation:
    input:
        ...
        db = rules.download_snpeff_db.output.done2
    output:
        vcf = "results/variants/{id}/variants.snpeff.vcf"
    params:
        db = "GRCh38.115",
...
rule filter_variants:
    ...
    - FORMAT/DP <10 : profondeur trop faible pour un appel fiable
